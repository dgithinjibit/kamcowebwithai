...
def ingest_documents_from_file(file_path):
    """Ingest documents from a file (PDF or text)"""
    ...
    all_embeddings, all_batch_content = concurrent_embed_documents(
        embed_model, docs, batch_size=50, max_workers=4
    )
    
    # Batch upsert vectors to Pinecone
    BATCH_SIZE = 100
    vectors_to_upsert = [
        (str(idx), embedding, {"text": content, "source": file_path})
    ...
    print(f"\nPinecone vector storage complete! Ingested {len(vectors_to_upsert)} chunks from {file_path}\n")

if __name__ == "__main__":
    # Example usage
    # Replace 'kamwenja_knowledge.txt' with your actual file path
    file_path = 'kamwenja_knowledge.txt'
    if os.path.exists(file_path):
        ingest_documents_from_file(file_path)
    else:
        print(f"File {file_path} not found. Create a text file with your knowledge base.")
